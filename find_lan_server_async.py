    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=2)) as session:
            async with session.get(f"http://{ip}") as response:
                ...
                if NAME in content:
                    print(f"{NAME} server found at {ip}")
                    return ip
                else:
                    print(f"Not a {NAME} server at {ip}.")
...
async def scan_network():
    print(f"Scanning subnet {SUBNET} for {NAME} server...")
    tasks = []
    for ip in ipaddress.IPv4Network(SUBNET, strict=False):
        ...
        tasks.append(asyncio.create_task(check_server(str(ip))))

    # Wait for all tasks to complete
    results = await asyncio.gather(*tasks)
    for result in results:
        if result:
            print(f"{NAME} server found at {result}")
            return

    print(f"{NAME} server not found on the local network.")
